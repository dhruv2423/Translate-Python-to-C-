...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	float total = 0.0;
      ...
      for(cell=0;cell<grid[0].size();cell++){
        total += grid[row][cell];
      }
    }
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	int height = grid.size();
  	...
  	float centre_prb = 1 - blurring;
  	float corner_prb = blurring/12;
  	float adj_prb = blurring/6;
  	vector < vector <float> > window = {
      {corner_prb, adj_prb, corner_prb},
      {adj_prb, centre_prb, adj_prb},
      {corner_prb, adj_prb, corner_prb}
    };
